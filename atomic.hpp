...
Cluster Crossover(Cluster cluster1, Cluster cluster2, float separation=1.0)
{
   ...
   float theta_,phi_;
   double max_z_aux;
   int clus1_bajo_z=0, clus1_sobre_z=0;
   int clus2_bajo_z=0, clus2_sobre_z=0;
   int contador_aux;
   ...
   for(i=0;i<cluster1.Nat;i++)
   {
      if(cluster1.atom[i].x[2]<0.0)
      {
         clus1_bajo_z++;
      }
      else
   ...
   contador_aux=0;
   if((clus1_bajo_z+clus2_sobre_z)==cluster1.Nat)
   {
      for(i=0;i<cluster1.Nat;i++)
      {
         if(cluster1.atom[i].x[2]<0.0)
   ...
   else if((clus1_bajo_z+clus2_sobre_z)>cluster1.Nat)
   {
      contador_aux=0;
      for(i=0;i<cluster1.Nat;i++)
      {
      ...
      i=0;
      while(contador_aux<cluster1.Nat)
      {
        if(cluster2.atom[i].x[2]>0.0)
        {
           for(j=0;j<3;j++)
   ...
   else if((clus1_bajo_z+clus2_sobre_z)<cluster1.Nat)
   {
     ...
     while(contador_aux<cluster1.Nat)
     {
        if((cluster1.atom[i].x[2]<(max_z_aux+0.5)) && (cluster1.atom[i].x[2]>max_z_aux) )
        {
           cluster3.atom[contador_aux].Symbol=cluster1.atom[i].Symbol;
           for(j=0;j<3;j++)
           {
        ...
        i++;
        if(i>cluster1.Nat){max_z_aux=max_z_aux+0.5; i=0;}
     }
   }
   return cluster3;
}
